...
public class IpV4Util
{
     ...
     //ip范围种类
     private static int IP_RANGE_TYPE_A=1;
     private static int IP_RANGE_TYPE_B=2;
     private static int IP_RANGE_TYPE_C=3;
     ...
     */
     ...
     /**
     * @param mask 任意的如"255.255.254.0"等格式，如果格式不合法，返回false
     */
     public boolean IpV4Util(String masks)
     {
    ...
    public static String trans2IpV4Str(byte[] ipBytes)
    {
      ...
      * 验证ip段是否合理：
      *  1.验证ip正则
	  *	 2.如果是ip段，判断前面的ip是否比后面的ip小，范围小的ip在前面  小-大：192.168.1.1-192.168.1.2
	  * ip范围有3种：
	  *  A. 192.168.1.1   一个ip地址
	  *  B. 192.168.1.1-192.168.1.2   以“-”分隔的ip段
	  *  c. 192.168.1.1-192.168.1.2,192.168.1.5-192.168.1.9  以“,”分隔的多个ip段
      * @param ipRange
      * @return boolean 如果3条全合理返回ture,只有要一条不合理返回false
      */
     public static boolean ipRangeValidate(String ipRange){
//    	 192.168.10.57-58
    	 return ipRangeRegex(ipRange) && compareIpRange(ipRange);
     }
     /**
      * 比较每个ip范围的开始和结束ip大小
      * 范围小的ip在前面  小-大：192.168.1.1-192.168.1.2
      * @param ipRange
      * @return
      */
     public	static boolean compareIpRange(String ipRange){
    	  String[] ipSplitc = ipRange.split(",");
			 boolean checkResult=false;
		    	if(ipSplitc.length>1){
		    		//C的情况
		    		for (String ipRangec : ipSplitc) {
		    			String ipRangecTrim = ipRangec.trim();
		    			String[] ips = ipRangecTrim.split("-");
		    			checkResult=compareIpRange(ips[0], ips[1]);
		    			if(!checkResult){
		    				break;
		    			}
		    		}
		    		...
		    		//A、B的情况
		    		String ipRangeTrim = ipRange.trim();
		    		String[] ipSplitAorB = ipRangeTrim.split("-");
		    		if(ipSplitAorB.length>1){
		    			//B的情况
		    			checkResult=compareIpRange(ipSplitAorB[0], ipSplitAorB[1]);
		    		}else{
		    			checkResult=true; 
		    		}
		    	}
     ...
     /**
      * 比较两个ip大小,如果start比end大，返回true，如果start比end小或者相等，返回false;
      * @param string
      * @param string2
      * @return
      */
     private static boolean compareIpRange(String start, String end) {
    	 
    	 String[] startSplit = start.split("\\.");
    	 String[] endSplit = end.split("\\.");
    	 int start1 = Integer.parseInt(startSplit[0]);
    	 int start2 =  Integer.parseInt(startSplit[1]);
    	 int start3 =  Integer.parseInt(startSplit[2]);
    	 int start4 =  Integer.parseInt(startSplit[3]);
    	 int end1 =  Integer.parseInt(endSplit[0]);
    	 int end2 =  Integer.parseInt(endSplit[1]);
    	 int end3 =  Integer.parseInt(endSplit[2]);
    	 int end4 =  Integer.parseInt(endSplit[3]);
    	 if(start1<end1){
    		 return true;
    	 }else if(start1==end1){
    		 if(start2<end2){
	...
	/**
      ...
      * @return boolean
      */
     public static boolean ipRangeSameSegmentByDefault(String ipRange){
    	 //只验证B、C两种情况的ip段
    	 String[] ipSplitc = ipRange.split(",");
		 boolean checkeResult=false;
	    	if(ipSplitc.length>1){
	    		//C的情况
	    		for (String ipRangec : ipSplitc) {
	    			String[] ips = ipRangec.split("-");
	    			checkeResult = checkSameSegmentByDefault(ips[0], ips[1]);
	    			if(!checkeResult){
	    				//只要有一个ip段不可合理，此ip段比合理，返回false
	    				break;
	    		...
	    		//A、B的情况
	    		String[] ipSplitAorB = ipRange.split("-");
	    		if(ipSplitAorB.length>1){
	    			//B的情况
	    			checkeResult=checkSameSegmentByDefault(ipSplitAorB[0], ipSplitAorB[1]);
	    		}else{
	    			checkeResult=true; 
	    		}
	    	}
      ...
      * @return
      */
    public static boolean ipRangeRegex(String ipRange){
    	String[] ipSplitc = ipRange.split(",");
    	...
    	if(ipSplitc.length>1){
    		...
    		for (String ipRangec : ipSplitc) {
    			...
    			String[] ips = ipRangecTrim.split("-");
    			if(ips.length<2){
    				boolean ipV4Validate = ipV4Validate(ips[0]);
    				return ipV4Validate;
    			}
    			boolean ipV4ValidateStart = ipV4Validate(ips[0]);
    			boolean ipV4ValidateEnd = ipV4Validate(ips[1]);
    			validateResult= ipV4ValidateStart && ipV4ValidateEnd;
    			if(!validateResult){
    				//只要有一个ip段不可合理，此ip段比合理，返回false
    				break;
    			}
    		...
    		if(ipSplitAorB.length>1){
    			//B的情况
    			boolean ipV4ValidateStart = ipV4Validate(ipSplitAorB[0]);
    			boolean ipV4ValidateEnd = ipV4Validate(ipSplitAorB[1]);
    			validateResult= ipV4ValidateStart && ipV4ValidateEnd;
    		}else{
    			//A的情况
    			validateResult= ipV4Validate(ipRangeTrim);
    		}
    	}
    	//其他情况不合理，返回false
    ...
    /**
     * 判断ip范围samllRange是否在bigRange范围内
     * 
     * scanIpRange1和scanIpRange2的范围格式有3种情况
	* 1.192.168.1.1   一个ip
	* 2.192.168.1.1-192.168.1.2   范围以“-”连接
	* 3.192.168.1.1-192.168.1.2,192.168.1.5-192.168.1.9  以“,”分隔的多个ip段
     * @param ipRange1 
     * @param ipRange2 
     * @return
     */
    public static boolean ipRange1InIpRange2(String bigRange,String samllRange){
    	boolean result=false;
    	return result;
    }
     /**
      * 判断ip范围的种类
      *  1. 192.168.1.1   一个ip地址
	  *  2. 192.168.1.1-192.168.1.2   以“-”分隔的ip段
	  *  3. 192.168.1.1-192.168.1.2,192.168.1.5-192.168.1.9  以“,”分隔的多个ip段
      * @param ipRange
      * @return int
      */
     public static int getIpRangeType(String ipRange){
